...
function makeWord(lettersObject) {
  const arr = new Array(Object.values(lettersObject));
...
class CssSelector {
  ...
  element(value) {
    ...
    newSelector.selectors = [...this.selectors, { type: 'element', value }];
  ...
  id(value) {
    ...
    if (lastSelector && !['element'].includes(lastSelector.type)) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
    ...
    const newSelector = new CssSelector();
    newSelector.selectors = [...this.selectors, { type: 'id', value }];
  ...
  class(value) {
    ...
    if (
      lastSelector &&
      ['attr', 'pseudoClass', 'pseudoElement'].includes(lastSelector.type)
    ) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    ...
    const newSelector = new CssSelector();
    newSelector.selectors = [...this.selectors, { type: 'class', value }];
    newSelector.hasElement = this.hasElement;
    newSelector.hasId = this.hasId;
    newSelector.hasPseudoElement = this.hasPseudoElement;
    return newSelector;
  ...
  attr(value) {
    ...
    if (
      lastSelector &&
      ['pseudoClass', 'pseudoElement'].includes(lastSelector.type)
    ) {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    ...
    const newSelector = new CssSelector();
    newSelector.selectors = [...this.selectors, { type: 'attr', value }];
    newSelector.hasElement = this.hasElement;
    newSelector.hasId = this.hasId;
    newSelector.hasPseudoElement = this.hasPseudoElement;
    return newSelector;
  ...
  pseudoClass(value) {
    const lastSelector = this.selectors[this.selectors.length - 1];
    if (lastSelector && lastSelector.type === 'pseudoElement') {
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    }
    ...
    const newSelector = new CssSelector();
    newSelector.selectors = [...this.selectors, { type: 'pseudoClass', value }];
    newSelector.hasElement = this.hasElement;
    newSelector.hasId = this.hasId;
    newSelector.hasPseudoElement = this.hasPseudoElement;
    return newSelector;
    ...
    this.selectors.forEach((selector) => {
      switch (selector.type) {
        ...
        case 'id':
          ...
          break;
        case 'class':
          classes += `.${selector.value}`;
          break;
        case 'attr':
          attrs += `[${selector.value}]`;
          break;
        case 'pseudoClass':
          pseudoClasses += `:${selector.value}`;
          break;
        case 'pseudoElement':
